---
title: "A Gender Inequalities Study "
author: "Anais HOAREAU, Sébastien MARCHAL, Rémi NAHON"
date: "`r format(Sys.time(), '%d %B %Y')`"
mail: "anais.hoareau@mines-ales.org, sebastien.marchal@mines-ales.org, remi.nahon@mines-ales.org"
linkedin: "rémi-nahon-609901160"
logo: "images/USA_logo.png"
output:
  epuRate::epurate:
    toc: TRUE
    number_sections: FALSE
    code_folding: "hide"
---

# Titre du rapport

<br><br>

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, tidy = TRUE, 
                      message = FALSE, fig.align = "center", eval = TRUE)
```


```{r import_libraries, echo = FALSE}
library(tidyverse)
library(rmarkdown)    
library(epuRate)      
library(dplyr)
library(ggplot2)
library(hrbrthemes)
library(shiny)
```


```{r data_load}
# Loading of the data
data <- read.csv("data/gender_inequality.csv")
# First visualization
head(data, 10)
#data$Gender.Inequality.Index..GII. <- as.numeric(levels(data$Gender.Inequality.Index..GII.))[data$Gender.Inequality.Index..GII.]
data$Gender.Inequality.Index..GII. <- as.numeric(as.character(data$Gender.Inequality.Index..GII.))
```
```{r lollipop}
data %>%
  filter(Gender.Inequality.Index..GII. != "..") %>%
  filter(!is.na(GII.Rank)) %>%
  arrange(desc(Gender.Inequality.Index..GII.)) %>%
  mutate(Country=factor(Country, Country)) %>%
  tail(20)%>%
  ggplot( aes(x=Country, y=Gender.Inequality.Index..GII.) ) +
    geom_segment( aes(x = Country, 
                      xend = Country,
                      y = 0,
                      yend = Gender.Inequality.Index..GII.),
                  color = "grey") +
    geom_point(size=3, color="#69b3a2") +
    coord_flip() +
    theme_ipsum() +
    theme(panel.grid.minor.y = element_blank(),
          panel.grid.major.y = element_blank()) +
    xlab("") +
    ylab("Gender Inequality Index")

data %>%
  filter(Gender.Inequality.Index..GII. != "..") %>%
  arrange(desc(Gender.Inequality.Index..GII.)) %>%
  mutate(Country=factor(Country, Country)) %>%
  head(20)%>%
  ggplot( aes(x=Country, y=Gender.Inequality.Index..GII.) ) +
    geom_segment( aes(x = Country, 
                      xend = Country,
                      y = 0,
                      yend = Gender.Inequality.Index..GII.),
                  color = "grey") +
    geom_point(size=3, color="#69b3a2") +
    coord_flip() +
    theme_ipsum() +
    theme(panel.grid.minor.y = element_blank(),
          panel.grid.major.y = element_blank()) +
    xlab("") +
    ylab("Gender Inequality Index")
```

```{r anais multichart}
# Basic ggplot
p <- data %>%
  mutate(myText=paste("This country is: " , Country )) %>%
  ggplot( aes(x= Population.with.Secondary.Education..Female., y=Percent.Representation.in.Parliament, size = Adolescent.Birth.Rate, color = Country, text=myText)) +
    geom_point(alpha=0.7) 

ggplotly(p, tooltip="text")
```

